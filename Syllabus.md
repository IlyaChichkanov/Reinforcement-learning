...
* Домашнее задание — раз в две недели, одно на блок из двух лекций, в формате `.ipynb` с автопроверкой части заданий через `assert`; сдаётся через GitHub Classroom
* Квиз — после каждой лекции, двенадцать вопросов на понимание; код результата сдаётся вместе с домашкой блока
...
* Оценка политики методом Монте-Карло; ценность состояния и действия на интуитивном уровне (уравнения Беллмана — неделя 2)
...
Задачи семинара — репетиция частей 1–2 домашки блока 1 на озере 4×4; решение — в [`solutions/week01/`](solutions/week01/seminar.ipynb).

* Интерфейс Gymnasium: `reset/step`, пространства состояний и действий на Frozen Lake; разминка — пройти озеро руками
* Политика как таблица: маршрут руками, что ломается на скользком льду
* Оценка политики методом Монте-Карло со стандартной ошибкой и ценность клетки (реализация с проверками)
* Метод Cross-Entropy по шагам: отбор элиты, обновление политики и цикл обучения, каждый шаг с проверками
* Эксперименты: сглаживание (кривые нескольких запусков), жадная версия политики, скользкий лёд, поле 8×8
...
* Двенадцать вопросов по лекции 1; код результата сдаётся вместе с домашним заданием блока 1 (неделя 2)
...
* Если бы у нас были оценки: ценность заданной стратегии $V^\pi$ и $Q^\pi$ и связь между ними как чередование ходов (в состоянии выбирает $\pi$ — усредняем, после действия выбирает среда — усредняем); затем лучшая стратегия: $Q^* = \max_\pi Q^\pi$, принцип оптимальности, жадный выбор, и связь $V^* = \max_a Q^*$ как частный случай (у жадной $\pi^*$ усреднение схлопывается в максимум); практическая разница $Q$ и $V$: почему алгоритмы без модели учат именно $Q$; демо — $Q^*$ и жадная стратегия против случайной
* Уравнение Беллмана: получается подстановкой одной связи в другую — сначала уравнение для заданной стратегии $\pi$, затем уравнение оптимальности (отличие ровно одно: усреднение по $\pi$ против максимума), для $Q$ и для $V$; проверка правой части на числах одной клетки; что именно считает одна итерация (формула против строки кода, явные циклы против векторной записи); метод простой итерации, он же value iteration; демо — «волна ценности» по итерациям и влияние платы за шаг на оптимальную стратегию
...
* Как оценить стратегию и как её улучшить: уравнение для $\pi$ из раздела 4 в работе, «два вопроса — два уравнения», оценка итерациями и матричная форма; шаг жадного улучшения и его гарантия, анонс policy iteration; демо — оценка случайной стратегии против Монте-Карло и один шаг улучшения
* Итоги: как DP, Monte-Carlo, TD, DQN, actor-critic и model-based методы решают то же уравнение

### Семинар

Среда берётся готовой из лекции; восемь коротких блоков `TODO`, под каждым проверка.
...
* Оценка заданной стратегии (вместо максимума — усреднение по $\pi$) итерациями и точно, одной линейной системой; сверка с Монте-Карло
* Один шаг улучшения и проверка, что ценность не упала ни в одной клетке; ★ policy iteration
* Лабиринт как `gym.Env`: пространства, `reset/step`, `check_env`, сверка частот переходов с моделью
* Эксперименты без кода: плата за шаг и сила ветра меняют оптимальную стратегию

Задачи семинара — репетиция частей 3–4 домашки блока 1; решение — в [`solutions/week02/`](solutions/week02/seminar.ipynb).

### Квиз

* Двенадцать вопросов по лекции 2

### Домашнее задание блока 1 (недели 1–2)

Одно задание на обе лекции блока, [`homeworks/hw1/homework.ipynb`](homeworks/hw1/homework.ipynb): 100 баллов и 10 бонусных. Каждая задача повторяет задачу семинара на новой среде: части 1–2 — семинар 1 на озере 8×8, части 3–4 — семинар 2 на складе.

* Cross-Entropy на Frozen Lake 8×8 (25): реализация и обучение, жадная политика и нулевые вероятности
* Оценка политики по Монте-Карло (10): три политики со стандартными ошибками
* Своя среда (20): «управление запасами» со случайным спросом как `gym.Env`, перебор уровня base-stock политики
* Уравнения Беллмана на складе (30): модель $P$, $R$ дана; ценность ручной политики точно и по Монте-Карло, оптимальная политика заказов через value iteration
* Теория (15): вывод уравнения Беллмана для $V^\pi$ и связи $V$–$Q$, порог в лотерее Колобка, склад как MDP с конечным горизонтом
* Бонус (10): спрос, зависящий от скрытого дня недели, и наблюдение, которое восстанавливает марковость
