...
## Неделя 2 (08.09): Математическая база RL: MDP, ценность, уравнения Беллмана. Своя среда как MDP

### Лекция

* Мост от лекции 1: Cross-Entropy учится по целым эпизодам; чтобы учиться с каждого шага, нужна ценность состояния
* MDP формально: $\langle S, A, P, R, \gamma \rangle$, траектория как случайный процесс, марковское свойство, наблюдение и состояние (POMDP на интуитивном уровне), терминальные состояния как поглощающие, граница агент–среда, награда как спецификация задачи; демо: матрицы $P$ и $R$ озера
* Политика фиксирует марковскую цепь ($P_\pi$, $r_\pi$); return, дисконтирование и тождество $G_t = R_t + \gamma G_{t+1}$
* Ценность состояния $V^\pi$ и ценность действия $Q^\pi$: определения, связь друг с другом; демо: тепловые карты ценности для двух политик
* Уравнения Беллмана: вывод, backup-диаграммы, матричная форма и решение линейной системы; демо: точное решение против Монте-Карло; итерации неподвижной точки и геометрическая сходимость (интуиция сжатия)
* Оптимальная политика: частичный порядок, теорема существования, $V^*$, $Q^*$, уравнения оптимальности, жадная политика; демо: value iteration на скользком озере и проверка по Монте-Карло; влияние $\gamma$ на оптимальную политику
* Одно уравнение — весь курс: как DP, Monte-Carlo, TD, Q-learning, DQN, actor-critic и model-based методы решают уравнение Беллмана; невязка Беллмана
...
* $Q^\pi$ из $V^\pi$ и один шаг жадного улучшения; value iteration на скользком поле, сравнение с Cross-Entropy из недели 1
...
* Теория: вывод уравнения Беллмана для $V^\pi$ и связи $V$–$Q$, расчёт ценности руками для цепочки состояний, формальное описание среды как MDP
...
* Повторение: уравнения Беллмана и уравнения оптимальности из недели 2
