...
Одно задание на обе лекции блока, [`homeworks/hw1/homework.ipynb`](homeworks/hw1/homework.ipynb): 100 баллов и до 30 бонусных. Весь ноутбук держится на одном наборе функций (`run_session`, `evaluate`, `cross_entropy_method`), которые пишутся в первых частях и дальше переиспользуются на складе.

* Cross-Entropy на Frozen Lake 8×8 (25): реализация, кривая обучения, сравнение вариантов сглаживания, скользкий лёд
* Оценка политики по Монте-Карло (10): три политики со стандартными ошибками, тепловая карта $V^\pi$
* Своя среда (20): «управление запасами» со случайным спросом как `gym.Env`, ручные base-stock политики
* Уравнения Беллмана на складе (25): точные $P$ и $R$ из распределения Пуассона, ценность ручной политики точно и по Монте-Карло, оптимальная политика заказов через value iteration, сравнение с ручной и с Cross-Entropy
* Теория (20): марковская цепь, вывод уравнения Беллмана для $V^\pi$ и связи $V$–$Q$, расчёт ценности руками, склад как MDP с конечным горизонтом, порог в лотерее Колобка
* Бонусы (по 10): спрос, зависящий от скрытого дня недели, и наблюдение, которое восстанавливает марковость; многорукие бандиты (ε-greedy, regret, UCB1); DL-разминка (упражнения из `dl_basics.ipynb`)
