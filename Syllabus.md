...
* Если бы у нас были оценки: оптимальная $Q^*(s, a)$, жадный выбор и принцип оптимальности Беллмана; связь $Q$ и $V$ как чередование ходов (в состоянии выбирает агент — максимум, после действия выбирает среда — среднее), обе формулы и практическая разница: почему алгоритмы без модели учат именно $Q$; демо — $Q^*$ и жадная стратегия против случайной
* Уравнение Беллмана: получается подстановкой одной связи в другую — отдельно уравнение для $Q^*$, отдельно для $V^*$; проверка правой части на числах одной клетки; что именно считает одна итерация (формула против строки кода, явные циклы против векторной записи); метод простой итерации, он же value iteration; демо — «волна ценности» по итерациям и влияние платы за шаг на оптимальную стратегию
...
* Как оценить стратегию и как её улучшить: $V^\pi$ и $Q^\pi$, та же пара связей при заданной $\pi$ (усреднение по стратегии вместо максимума) и «два вопроса — два уравнения», оценка итерациями и матричная форма; шаг жадного улучшения и его гарантия, анонс policy iteration; демо — оценка случайной стратегии против Монте-Карло и один шаг улучшения
