...
## Неделя 2 (08.09): Постановка задачи RL, оценочные функции и уравнение Беллмана. Своя среда как MDP

### Лекция

Изложение следует главе «Обучение с подкреплением» Учебника по машинному обучению ШАД; сквозной пример — Frozen Lake, все формулы проверяются на нём числами.

* Обучение методом проб и ошибок: награда как «разметка» (тортики, велосипед), три свойства награды; чего не хватает Cross-Entropy
* Постановка задачи: MDP $(S, A, p, r)$, стратегия $\pi(a \mid s)$, эпизод и траектория, среда как управляемая марковская цепь, три допущения (марковость, стационарность, полная наблюдаемость / POMDP), цель как средняя дисконтированная сумма наград, return и тождество $G_t = r_t + \gamma G_{t+1}$; примеры MDP (табличные миры, видеоигры, непрерывное управление); демо: функция переходов озера
* Окей, и как такое решать: структура задачи (подзадача той же структуры из $s'$), оптимальная Q-функция $Q^*$, принцип оптимальности Беллмана, жадная стратегия и её детерминированность; демо: $Q^*$ озера и жадная стратегия
* Уравнение оптимальности Беллмана: вывод на конкретной клетке с числами, затем в общем виде; единственность решения, метод простой итерации и геометрическая сходимость; демо: арифметика для клетки 14, простая итерация с нуля
* Оценочные функции заданной стратегии $V^\pi$, $Q^\pi$: связь друг с другом, линейное уравнение Беллмана, «два уравнения — два вопроса»; демо: решение линейной системы против Монте-Карло
* А где же метод проб и ошибок: переходы $(s, a, r, s')$, Беллмановский таргет, сглаживание с $\alpha$, формула Q-learning, пример «Колобок и лотерея», exploration / exploitation и $\varepsilon$-жадная стратегия, алгоритм Q-learning; демо: лотерея, Q-learning на озере
* Что дальше: как DP, Monte-Carlo, TD, DQN, actor-critic и model-based методы решают уравнение Беллмана

### Семинар

* Своя среда GridWorld как класс `gym.Env` (каркас с `TODO`, `check_env`, ручной маршрут) — конкретный MDP
* Модель MDP из среды: матрицы $P[s, a, s']$ и $R[s, a]$ по карте и параметру скольжения, проверки
* Оценка политики тремя способами: Монте-Карло, точное решение $(I - \gamma P_\pi)^{-1} r_\pi$, итерации Беллмана; совпадение ответов, тепловые карты
* $Q^\pi$ из $V^\pi$ и один шаг жадного улучшения; value iteration на скользком поле, сравнение с Cross-Entropy из недели 1; Q-learning на своём поле
...
* Повторение: уравнение оптимальности Беллмана, метод простой итерации и Q-learning из недели 2
