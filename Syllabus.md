...
* Агент, среда и граница между ними как проектное решение; разминка «разложите задачу на части»
* Наблюдение и состояние (POMDP на интуитивном уровне): демо на CartPole без скорости, стек кадров как лекарство (`FrameStackObservation`), частично наблюдаемый GridWorld
* Действия дискретные, непрерывные (демо `Pendulum-v1`, дискретизация) и составные; маска допустимых действий
* Награда: разреженная и плотная, shaping, потенциальные функции, типичные ошибки при проектировании награды и самопроверка «найдите ошибку»
* Политика: детерминированная и стохастическая (демо «камень-ножницы-бумага»), табличная и параметрическая; эпизодические и непрерывные задачи
* Устройство среды в Gymnasium: `gym.Env`, `spaces`, `reset/step`, `render`, `check_env`; своя среда GridWorld
* Обёртки (`TimeLimit`, `RecordEpisodeStatistics`, свои `RewardWrapper`/`ObservationWrapper`); эксперимент «три награды и одна ловушка»: разреженная, «+0.1 за шаг», потенциальный shaping — с честной оценкой
* Векторные среды и воспроизводимость: сиды, `make_vec`, `gym.register`; обзор готовых коллекций сред (Gymnasium, ALE, MiniGrid, PettingZoo, Isaac Lab и др.)

### Семинар

* Строим свою среду: GridWorld с препятствиями и стохастическими переходами как класс `gym.Env`; проверка `check_env`, случайный и ручной агент, маска действий
* Обёртки: ограничение длины эпизода, `StepPenalty`, `PotentialShaping` своими руками с проверкой телескопичности, запись статистики
* Cross-Entropy из недели 1 на своей среде и на скользком полу; ловушка наивного shaping (бонус за приближение) с честной оценкой
* Частичная наблюдаемость на CartPole: `ObservationWrapper`, прячущий скорость, и `FrameStackObservation`
...
* Теория: вывести уравнение Беллмана для $V^\pi$ из определения return; показать, что оператор Беллмана — сжатие в норме $\|\cdot\|_\infty$
