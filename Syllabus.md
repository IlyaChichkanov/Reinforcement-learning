...
Изложение следует главе «Обучение с подкреплением» Учебника по машинному обучению ШАД. Сквозной пример — клетчатый мир 3 × 4 из учебника (выход +1, яма −1, плата за шаг, ветер 0.8 / 0.1 / 0.1); все формулы проверяются на нём числами, кода в лекции минимум.

* Где мы остановились: Cross-Entropy оценивает эпизод целиком, 99 верных шагов выбрасываются вместе с одной ошибкой; хотим оценку на каждый шаг
* Мир, на котором всё посчитаем: правила клетчатого мира, ветер, плата за шаг, конец эпизода; демо — переходы одного действия и эпизоды случайной стратегии
* Если бы у нас были оценки: оптимальная $Q^*(s, a)$, жадный выбор и принцип оптимальности Беллмана; разница между $Q$ и $V$ и почему алгоритмы без модели учат именно $Q$; демо — $Q^*$ и жадная стратегия против случайной
* Уравнение Беллмана: вывод на одной клетке с числами, затем в общем виде; метод простой итерации (value iteration); демо — «волна ценности» по итерациям и влияние платы за шаг на оптимальную стратегию
...
* Насколько хороша заданная стратегия: $V^\pi$ и $Q^\pi$, «два вопроса — два уравнения» ($\max_a$ против усреднения по $\pi$), линейная система; демо — точное решение против Монте-Карло
* Когда модель неизвестна: переход $(s, a, r, s')$, Беллмановский таргет, сдвиг с шагом $\alpha$, пример «Колобок и лотерея», $\varepsilon$-жадная стратегия; демо — Q-learning на том же мире без знания модели
